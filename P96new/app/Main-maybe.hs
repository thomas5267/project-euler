...
newtype Sudoku = Sudoku { unSudoku :: U.Vector (Value Sudoku)
                        } deriving (Show, Eq)
...
newtype SudokuSlice = SudokuSlice { unSudokuSlice :: U.Vector (Value Sudoku)
                                  } deriving (Show, Eq)
...
instance SudokuShaped SudokuPage where
  ...
  update s x = SudokuPage { rowPage = update (rowPage s) x
                          , colPage = update (colPage s) x
                          , boxPage = update (boxPage s) x
                          }
  updates s xs = SudokuPage { rowPage = updates (rowPage s) xs
...
instance SudokuShaped SudokuState where
  ...
  update s x = SudokuState { board = update (board s) x
                           , page = update (page s) x
                           }
  updates s xs = SudokuState { board = updates (board s) xs
...
checkCompleted :: SudokuState -> Bool
...
findGoodCell :: (SudokuIndex i) => SudokuState -> Maybe (i, [Value Sudoku])
findGoodCell s = foldr1 go
    . filter (not . null . snd)
    <$> traverse (availableNum s) rowMajorTraversal
  where
    isGood [_, _] = True
...
fill :: SudokuState -> Maybe SudokuState
fill s0 = deduce s0 >>= core
  where
    core :: SudokuState -> Maybe SudokuState
    core s
      | checkCompleted s = pure s
      | otherwise = asum . map core . nextStates $ s

    nextStates :: SudokuState -> [SudokuState]
    nextStates s = do
        (i, cs) <- maybeToList $ findGoodCell @Row s
        mapMaybe (deduce . (\ c -> s `update` (i, c))) cs
...
extractor :: Maybe SudokuState -> Int
...
main = do
    ...
    print $ foldl' (+) 0 . map (extractor . fill) . toSudokuList $ file
  where
    toSudokuList :: String -> [SudokuState]
    -- toSudokuList = map parseSudoku . concat . chunksOf 9 . trimFile . lines
    toSudokuList = map parseSudoku . divvy 81 88 . filter (/= '\n') . drop 8
