...
availableNum i = do
    ...
    then let candidates = map ((+1) . fromIntegral) . listBits . unSudokuSet $ set
          in case candidates of
            ...
            _  -> return (i, candidates)
    else return (i, [])


checkCompleted :: SudokuProcessor Sudoku
checkCompleted = do
    completed <- gets (U.notElem 0 . unSudoku . board)
    unless completed $ throwError Incomplete
    gets board


deduce :: SudokuProcessor ()
deduce = do
    updates <- mapMaybe (traverse onlyOneChoice)
        <$> traverse (availableNum @Row) rowMajorTraversal
    when (null updates) $ throwError Incomplete
    if isConsistent updates
    then modify' (`update` updates)
    else throwError Inconsistent
  where
    ...
    isConsistentRow us = all go $ zip vs (tail vs)
      where
        vs = map (first convertIndex) us
        go ((Row (rx, _), dx), (Row (ry, _), dy)) = rx /= ry || dx /= dy

    isConsistentCol :: (SudokuIndex i) => [(i, Int8)] -> Bool
    isConsistentCol us = all go
                       . (\ u -> zip u (tail u))
                       . sort
                       $ map (first convertIndex) us
      where
        go ((Col (cx, _), dx), (Col (cy, _), dy)) = cx /= cy || dx /= dy

    isConsistentBox :: (SudokuIndex i) => [(i, Int8)] -> Bool
    isConsistentBox us = all go
                       . (\ u -> zip u (tail u))
                       . sort
                       $ map (first convertIndex) us
      where
        go ((Box (bx, _), dx), (Box (by, _), dy)) = bx /= by || dx /= dy

deduceAll :: SudokuProcessor Sudoku
deduceAll = forever deduce `catchError` continueOnNoDeduce >> checkCompleted
  where
    continueOnNoDeduce :: SudokuError -> SudokuProcessor ()
...
findGoodCell :: (SudokuIndex i) => SudokuProcessor (i, [Int8])
...
guess :: (SudokuIndex i) => SudokuChoice i -> SudokuProcessor ()
...
-- fill :: SudokuProcessor Sudoku
-- fill = do
--     (checkCompleted >> deduceAll >> core) `catchError` whenCompleted
--     gets board
--   where
--     whenCompleted :: SudokuError -> SudokuProcessor ()
--     whenCompleted e = case e of
--         Completed -> return ()
--         _         -> throwError e
-- 
--     core :: SudokuProcessor ()
--     core = do
--         (i, cs) <- findGoodCell @Row
--         save <- get
--         let revertOnFail :: SudokuError -> SudokuProcessor ()
--             revertOnFail e = case e of
--                 Completed -> throwError e
--                 _         -> put save
--         forM_ cs (\ c -> (guess (i, c) >> deduceAll >> core)
--             `catchError` revertOnFail)
--         throwError Inconsistent

fill :: SudokuProcessor Sudoku
fill = checkCompleted `catchError` whenIncomplete core
  where
    whenIncomplete :: SudokuProcessor Sudoku
                   -> SudokuError
                   -> SudokuProcessor Sudoku
    whenIncomplete f e = case e of
        Incomplete -> f
        _          -> throwError e

    -- core <-> try <o
    core :: SudokuProcessor Sudoku
    core = deduceAll `catchError` whenIncomplete (findGoodCell @Row >>= try)

    try :: (SudokuIndex i) => (i, [Value Sudoku]) -> SudokuProcessor Sudoku
    try (_, []) = throwError Inconsistent
    try (i, c:cs) = do
        save <- get
        let tryNextOnInconsistent :: SudokuError -> SudokuProcessor Sudoku
            tryNextOnInconsistent e = case e of
                Incomplete -> core
                _          -> put save >> try (i, cs)
        (guess (i, c) >> core) `catchError` tryNextOnInconsistent
...
extractor :: Either SudokuError Sudoku -> Int
extractor (Right s) = toInt $ U.slice 0 3 $ unSudoku s
  where
